...
void MKLDNNPoolingGradCompute(const OpContext &ctx, const PoolingParam &param,
  ...
  if (req != kAddTo) {
    auto diff_src_mem = in_grad.GetMKLDNNDataReorder(bwd.pd.diff_src_desc());
    mkldnn_args_map_t args = {{MKLDNN_ARG_DIFF_DST, *(out_grad.GetMKLDNNData())},
                              {MKLDNN_ARG_DIFF_SRC, *diff_src_mem}};
